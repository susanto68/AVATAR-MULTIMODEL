// UMD script: uses global THREE and THREE.GLTFLoader
// Converted from script.js to avoid ESM on Vercel static hosting
...
const HUGGINGFACE_BASE = 'https://text.pollinations.ai/';
...
function updateLivePose(dt, t) {
	...
	if (blinkTimer > 0) {
		...
		blinkIntensity = 1 - Math.abs(2 * n - 1);
	}
...
function setupUI() {
	...
	input.addEventListener('keydown', (e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') onAsk(); });
	try {
...
function onAsk() {
	...
	queryWithFallback(prompt, modelSelect.value)
		.then((reply) => { pushHistory({ role: 'assistant', content: reply }); renderHistory(); speak(reply); })
		.catch((err) => { console.error(err); const msg = 'All models unavailable right now. Please try again later.'; pushHistory({ role: 'assistant', content: msg }); renderHistory(); toast(msg, true); })
	...
	for (const model of order) {
		try {
			...
			if (reply) {
				...
				if (trimmed && !/^Ah, Llama 3\.1/i.test(trimmed)) { statusChip.textContent = `${model} ✓`; return trimmed; }
			}
			throw new Error(`Empty reply from ${model}`);
		} catch (e) { lastError = e; console.warn(`Model ${model} failed`, e); }
	}
...
function speak(text) {
	...
	const shouldUseResponsiveVoice = () => { try { if (location.protocol !== 'https:') return false; return !!(window.responsiveVoice && window.responsiveVoice.speak); } catch { return false; } };
	const useWebSpeech = () => {
		try { const synth = window.speechSynthesis; if (!synth) throw new Error('No speechSynthesis'); const uttr = new SpeechSynthesisUtterance(text); uttr.onstart = () => { onstart(); startMouthMotion('webspeech', text, uttr); }; uttr.onend = () => { onend(); stopMouthMotion(); }; uttr.onboundary = () => mouthBeat(); const voices = synth.getVoices?.() || []; const preferred = voices.find(v => /en-GB/i.test(v.lang) && /male/i.test(v.name)) || voices.find(v => /en-GB/i.test(v.lang)) || voices.find(v => /English/i.test(v.name)); if (preferred) uttr.voice = preferred; synth.cancel(); synth.speak(uttr); } catch (e) { console.warn('TTS unavailable', e); }
	};
	let started = false;
	try {
		if (shouldUseResponsiveVoice()) {
			window.responsiveVoice.cancel();
			window.responsiveVoice.speak(text, 'UK English Male', { rate: 1, onstart: () => { started = true; onstart(); startMouthMotion('rv', text); }, onend: () => { onend(); stopMouthMotion(); } });
			setTimeout(() => { if (!started) { try { window.responsiveVoice.cancel(); } catch {} useWebSpeech(); } }, 400);
			return;
		}
	} catch {}
	useWebSpeech();
...
function startListening() {
	try { const SR = window.SpeechRecognition || window.webkitSpeechRecognition; if (!SR) throw new Error('SpeechRecognition unavailable'); if (!recognition) { recognition = new SR(); recognition.continuous = false; recognition.interimResults = true; recognition.lang = 'en-US'; recognition.onstart = () => setListening(true); recognition.onerror = (e) => { console.warn('SR error', e); setListening(false); }; recognition.onend = () => setListening(false); recognition.onresult = (event) => { let interim = ''; let finalText = ''; for (let i = event.resultIndex; i < event.results.length; i++) { const res = event.results[i]; if (res.isFinal) finalText += res[0].transcript; else interim += res[0].transcript; } if (interim) input.value = interim; if (finalText.trim()) { input.value = finalText.trim(); onAsk(); } }; }
		recognition.start(); statusChip.textContent = 'Listening…'; } catch (e) { console.warn('Could not start SR', e); }
}

function stopListening() { try { if (recognition) recognition.stop(); } catch {} setListening(false); }

function setListening(v) { isListening = !!v; if (micBtn) micBtn.textContent = v ? '■' : '🎤'; statusChip.textContent = v ? 'Listening…' : 'Ready'; }

function startMouthMotion(source, text, uttr) { stopMouthMotion(); mouth.target = 0; mouth.open = 0; if (uttr && typeof uttr.addEventListener === 'function') { uttr.addEventListener('boundary', () => mouthBeat()); } mouthInterval = setInterval(() => { mouth.target *= 0.85; }, 50); mouthBeat(); }
function stopMouthMotion() { if (mouthInterval) clearInterval(mouthInterval); mouthInterval = null; mouth.target = 0; }
function mouthBeat() { const base = 0.6; const jitter = Math.random() * 0.3; mouth.target = Math.min(1, base + jitter); }

function setSpeaking(v) { speakingAnimationActive = !!v; speaking.classList.toggle('hidden', !v); }
function setLoading(v) { spinner.classList.toggle('hidden', !v); askBtn.disabled = !!v; }

function pushHistory(msg) { lastFive.push({ role: msg.role, content: msg.content }); while (lastFive.length > 5) lastFive.shift(); localStorage.setItem('avatar_history', JSON.stringify(lastFive)); }
function loadHistory() { try { const s = localStorage.getItem('avatar_history'); return s ? JSON.parse(s) : []; } catch { return []; } }
function renderHistory() { historyEl.innerHTML = ''; for (const m of lastFive) { const div = document.createElement('div'); div.className = 'msg ' + (m.role === 'user' ? 'user' : 'bot'); div.textContent = m.content; historyEl.appendChild(div); } historyEl.scrollTop = historyEl.scrollHeight; }
function toast(text, isError) { statusChip.textContent = text; statusChip.style.color = isError ? 'var(--danger)' : 'var(--accent)'; setTimeout(() => { statusChip.textContent = 'Ready'; statusChip.style.color = 'var(--accent)'; }, 3500); }
function bootstrapWelcome() { const welcome = 'Hello! I am your AI avatar. Ask me anything.'; pushHistory({ role: 'assistant', content: welcome }); renderHistory(); setTimeout(() => speak(welcome), 400); }
